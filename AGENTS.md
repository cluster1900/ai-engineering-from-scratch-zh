...
- **课中检查：** 每节课在关键概念、公式推导、代码实现或框架切换之后，穿插 2-4 个短问题，用来观察学习者是否真的跟上。问题应聚焦常见误解、边界条件或“为什么这样做”，不要只考术语背诵。
- **课后深练习：** 每节课结尾必须安排 2-3 个有深度的课后题或小作业，帮助学习者把本课 artifact 迁移到新场景。优先设计需要修改代码、解释现象、比较方案、诊断错误或扩展实现的任务，并给出清晰的验收标准或预期观察。
- **Lesson 结束巩固：** 每节课讲解结束后必须有一个明确的收束段，帮助学习者把本课内容沉淀下来。至少包含本课核心概念回顾、关键术语复盘、1-2 个自检问题，以及它如何连接到下一节课；Build lesson 还要说明本课 artifact 能做什么、如何验证它已经跑通。
- **Phase 综合考核：** 每个 phase 完成后，必须提供一组更大的综合题，用来检查学习者对本 phase 的整体掌握。综合题应跨越多节课，覆盖数学理解、手写实现、framework 使用、调试能力和设计取舍；可以放在该 phase 的 capstone lesson、最后一节 lesson，或专门的 phase review artifact 中。
- **深度优先：** 所有检查题都服务于掌握程度判断。避免“看过就能答”的记忆题，优先让学习者解释原因、预测结果、定位 bug、改造 artifact，或把同一个概念迁移到未见过的数据和约束下。
