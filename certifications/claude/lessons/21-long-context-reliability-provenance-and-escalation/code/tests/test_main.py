...
class ReliabilityPacketTests(unittest.TestCase):
    ...
    def test_coverage_count_is_required(self):
        text = without_contract_terms(self.text, "18 of 24", "omitted")
    ...
    def test_provenance_location_is_required(self):
        self.assertEqual(main.validate_text(without_contract_terms(self.text, "location"))["status"], "blocked")

    def test_conflict_owner_is_required(self):
        text = without_contract_terms(self.text, "owner", "safe next action")
    ...
    def test_random_sample_is_required(self):
        self.assertEqual(main.validate_text(without_contract_terms(self.text, "random sample"))["status"], "blocked")
