...
class DiscoveryBriefTests(unittest.TestCase):
    ...
    def test_baseline_and_target_are_required(self):
        text = without_contract_terms(self.text, "baseline", "target")
    ...
    def test_assumptions_section_is_required(self):
        self.assertEqual(main.validate_text(without_contract_terms(self.text, "## Assumptions"))["status"], "blocked")

    def test_non_goal_is_required(self):
        self.assertEqual(main.validate_text(without_contract_terms(self.text, "autonomous sending"))["status"], "blocked")
