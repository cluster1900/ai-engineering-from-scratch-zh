...
class SessionRecoveryPacketTests(unittest.TestCase):
    ...
    def test_durable_state_is_required(self):
        self.assertEqual(main.validate_text(without_contract_terms(self.text, "## Durable State"))["status"], "blocked")
    ...
    def test_unknown_outcome_needs_idempotency(self):
        text = without_contract_terms(self.text, "idempotency", "unknown outcome")
