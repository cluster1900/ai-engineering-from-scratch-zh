(function () {
  ...
  var STAGES = [
    { id: 'foundations', number: '01', name: '基础', startTier: 0, endTier: 3, focusPhase: 0 },
    { id: 'model-disciplines', number: '02', name: 'Model 学科', startTier: 4, endTier: 6, focusPhase: 7 },
    { id: 'engineering-systems', number: '03', name: '工程系统', startTier: 7, endTier: 11, focusPhase: 11 },
    { id: 'capstone-proof', number: '04', name: 'Capstone 成果验证', startTier: 12, endTier: 12, focusPhase: 19 }
  ];
  ...
  function phaseNarration(phase, state, progress) {
    ...
    var text = '阶段 ' + formatPhase(phase.id) + '：' + phase.name + '。' + state.label + '。已完成 ' +
      progress.total + ' 节课程中的 ' + progress.done + ' 节。';
    text += requirements.length ? ' 直接先修阶段：' + requirements.join('、') + '。' : ' 这是起始阶段。';
    text += unlocks.length ? ' 可立即解锁：' + unlocks.join('、') + '。' : ' 这是最终目标。';
    return text;
  }
  ...
  function updateNodeRouteState(nodeId, nextState) {
    ...
    if (nextState === 'selected') setNodeState(node, '已选择', true);
    else if (nextState === 'prerequisite') setNodeState(node, '先修阶段', true);
    else if (nextState === 'unlock') setNodeState(node, '可解锁', true);
  }
  ...
  function restoreSelectionFromUrl(shouldCenter) {
    ...
    if (!phaseMap[id]) {
      if (selectedId !== null) clearSelection(false);
      setText('roadmapGraphStatus', '此链接没有对应的路线图阶段。');
      return;
    }
  ...
  function renderEmptyInspector(animate) {
    ...
    var recommendationHtml = recommendation
      ...
      : '';
    updateInspector(
      '<span class="roadmap-inspector-eyebrow">路线检查器</span>' +
      '<h2>选择一个阶段</h2>' +
      '<p class="roadmap-inspector-copy">选择一个节点，即可突出显示通往该节点的确切路线、它能够解锁的所有阶段，以及根据你的本地进度最适合继续学习的课程。</p>' +
      recommendationHtml,
      !!animate
    );
  }
  ...
  function renderInspector(id, animate) {
    ...
    updateInspector(
      ...
      '<div class="roadmap-inspector-context">' +
        ...
        '<div class="roadmap-inspector-stat"><strong>' + Object.keys(descendants).length + '</strong><span class="roadmap-inspector-stat-label">已解锁阶段</span></div>' +
      '</div>' +
      '<div class="roadmap-route-sections">' +
        renderRouteSection('直接先修阶段', directPrereqs, '这是起点。') +
        renderRouteSection('可立即解锁', directUnlocks, '这是最终目标。') +
      '</div>' +
      '<div class="roadmap-actions">' +
        (lessonLink ? '<a class="roadmap-action roadmap-action-primary" href="' + lessonLink + '">' + actionLabel + '</a>' : '') +
        '<a class="roadmap-action" href="' + phaseGithubUrl(phase) + '" target="_blank" rel="noopener">在 GitHub 上查看阶段</a>' +
      '</div>',
      animate !== false
    );
  }
  ...
  function announceSelection(id) {
    ...
    setText('roadmapGraphStatus', '已选择阶段 ' + formatPhase(id) + '。已突出显示 ' + ancestors + ' 个先修阶段和 ' + descendants + ' 个下游阶段。');
  }
  ...
  function showDataError() {
    var wrap = document.getElementById('roadmapGraphWrap');
    if (wrap) wrap.innerHTML = '<p>无法加载路线图数据。请重新构建站点并刷新此页面。</p>';
  }
