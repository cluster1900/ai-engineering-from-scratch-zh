...
test('repository Agent Skills path routes 22 to 24 and keeps 23 optional', () => {
  ...
  assert.equal(poisoningPreflight.title, 'Tool poisoning 与不可信指令');
...
test('optional MCP capstone keeps its prerequisite gate in every lesson reader surface', () => {
  ...
  assert.match(lessonHtml, /learningPathPrerequisiteCallout\(focusedEntry, '本课开始前必须完成'\)/);
  ...
  assert.match(lessonHtml, /focusedOptionalLocked \? '已锁定的可选 Capstone：' : '可选 Capstone：'/);
});
...
test('generic course skills dispatch every supported state to an installed owner', () => {
  ...
  for (const name of ['learn', 'start-learning']) {
    ...
    const section = source.match(/## (?:Focused Agent Skills handoff|专注型 Agent Skills 交接|专注的 Agent Skills 交接)\s+([\s\S]*?)(?=\n## |$)/);
    ...
    assert.match(section[1], /(?:do not\s+(?:copy Agent Skills state into|create)|不要[\s\S]*?(?:复制到|创建))\s*`LEARNING\.md`/);
    const resume = source.match(/## (?:Resume routing across course modes|跨课程模式的恢复路由)\s+([\s\S]*?)(?=\n## |$)/);
    assert.ok(resume, `${name} is missing cross-route resume handling`);
    for (const [stateFile, owner] of routeOwners) {
      assert.match(
        resume[1],
        new RegExp('`' + stateFile.replace('.', '\\.') + '` (?:belongs to|属于|由)[^\n]*`' + owner + '`'),
    ...
    assert.match(
      resume[1],
      /`MCP-ENGINEERING-LEARNING\.md`[\s\S]*?`learn-mcp`[\s\S]*?(?:legacy|旧版)[\s\S]*?(?:not a separate|并非独立|不是单独)/
    );
    assert.match(resume[1], /(?:指定了路线|指定了路径)[\s\S]*?即使[\s\S]*?分派给|names a route[\s\S]*?dispatch to[\s\S]*?even when/);
    assert.match(resume[1], /(?:收集[\s\S]*?(?:所有者|负责人)|group the files by route owner|collect the owners whose state files\s+exist)/);
    assert.match(resume[1], /(?:恰好只剩一个|exactly one route)[\s\S]*?(?:恢复|调用|resume|invoke)/);
    assert.match(resume[1], /(?:两个或更多|two\s+or more)/);
    assert.match(resume[1], /(?:询问要恢复哪|ask which\s+(?:one|route)\s+to\s+resume)/);
    assert.match(source, /(?:Legacy runtime|旧版运行时)[\s\S]*?`learn-mcp-engineering`[\s\S]*?(?:alias|别名)[\s\S]*?`learn-mcp`/i);
    ...
    const genericStart = name === 'learn'
      ? Math.max(source.indexOf('## Step 0'), source.indexOf('## 步骤 0'))
      : Math.max(source.indexOf('If `LEARNING.md` already exists'), source.indexOf('如果 `LEARNING.md` 已存在'));
    const resumeStart = Math.max(
      source.indexOf('## Resume routing across course modes'),
      source.indexOf('## 跨课程模式的恢复路由')
    );
    assert.ok(resumeStart >= 0 && resumeStart < genericStart);
  }
...
test('course guide shape count matches its six routing bullets in both mirrors', () => {
  ...
  for (const file of [
    ...
    const source = fs.readFileSync(file, 'utf8');
    const routing = source.match(/1\. \*\*(?:Interpret the ask|理解请求)\*\*[\s\S]*?(?=\n2\. \*\*(?:Scan the Contents tables|扫描目录表)\*\*)/);
    assert.ok(routing, `${file} is missing the routing-shape section`);
    assert.match(routing[0], /(?:one of six shapes|以下六种形式之一)/);
...
test('homepage routes loop, graph, and harness engineering to real lessons', () => {
  ...
  const routes = [
    ['循环工程', 'phases/14-agent-engineering/01-the-agent-loop'],
    ['图工程', 'phases/14-agent-engineering/13-langgraph-stateful-graphs'],
    ['Harness 工程', 'phases/14-agent-engineering/31-agent-workbench-why-models-fail'],
  ];
  ...
  assert.match(homepage, /构建 Agent 状态图编排/);
});
...
test('homepage preserves live GitHub CTAs and the motion-aware learner marquee', () => {
  ...
  assert.match(mastheadCta[0], /<span>选择你的目标<\/span>/);
  assert.doesNotMatch(mastheadCta[0], /Start (?:MCP Engineering|Agent Skills)/i);
  assert.match(
    mastheadCta[0],
    /<a class="masthead-btn" href="https:\/\/github\.com\/rohitg00\/ai-engineering-from-scratch"[^>]*aria-label="在 GitHub 上为 ai-engineering-from-scratch 加 Star"[^>]*>[\s\S]*?<span>在 GitHub 上加 Star<\/span>[\s\S]*?<span class="masthead-btn-count" data-gh-stars="rohitg00\/ai-engineering-from-scratch" data-loading="true">/
  );
  assert.match(
    mastheadCta[0],
...
test('website motion contracts keep interaction state stable and compositor-friendly', () => {
  ...
  assert.match(agentLoop[0], /第 ' \+ \(state\.step \+ 1\) \+ ' 步，共 12 步/);
...
test('lesson reader keeps learning-path context and renders a copyable full-depth install', () => {
  ...
  assert.match(lessonHtml, /第 ' \+ \(focusedIndex \+ 1\) \+ ' 课，共 ' \+ focusedLessons\.length/);
  ...
  assert.match(
    lessonHtml,
    /data-learning-path-gate-label>'\s*\+\s*\(nextLocked\s*\?\s*'已锁定'\s*:\s*'下一课 &rarr;'\)/
  );
  ...
  assert.match(lessonHtml, /需要本地 clone/);
  ...
  assert.match(lessonHtml, /在 Codex 中使用 <code>check-understanding /);
  assert.match(lessonHtml, /\/check-understanding/);
  assert.match(lessonHtml, /动手完成本课/);
  ...
  assert.match(lessonHtml, /请从 repository 根目录（包含 README\.md 的文件夹）运行/);
  assert.match(lessonHtml, /请从 repository 根目录（包含 README\.md 和 phases\/ 的目录）运行复制的命令/);
  ...
  assert.match(lessonHtml, /代码围栏并不自动代表可运行程序/);
...
test('MCP evaluators expose each protocol boundary in its owning scenario', () => {
  ...
  assert.equal(drift.evidence.identityRule, '显示名称和 serverInfo 不是安全身份');
...
test('reduced motion holds SMIL figures on a meaningful static frame', () => {
  ...
  assert.equal(control.getAttribute('aria-label'), '由于已启用减少动态效果，Animation 已禁用');
  assert.equal(runtime.scheduledFrames.size, 0);
});
...
test('MCP contract evaluator follows empty cursors and validates every structuredContent JSON type', () => {
  ...
  assert.match(emptyCursor.verdict, /即使它是空字符串/);
  ...
  assert.match(mismatch.verdict, /不会免除 outputSchema/);
...
test('MCP progress is server-to-client and every reliability Task snapshot is complete', () => {
  ...
  assert.equal(stream.evidence.response.progressDirection, '在请求作用域响应上从 Server 到客户端');
...
test('MCP registry drift quarantines and deactivates only the drifted release', () => {
  ...
  assert.equal(result.evidence.rollbackCandidate.activationRequires, '显式回滚决策');
  assert.match(result.verdict, /已单独准入且健康的 3\.9\.2 release/);
});
