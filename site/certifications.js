(function () {
  ...
  function renderInternalLessonReference(ref, sourceTrack) {
    ...
    if (crossTrack) label += ' · ' + (sourceTrack.examCode || sourceTrack.shortName || sourceTrack.id) + ' 的补充内容';
  ...
  function passing(track) {
    return examValue(track, ['passingScaledScore', 'passingScore', 'passingScaled'], '请参阅官方指南');
  }

  function price(track) {
    var value = examValue(track, ['price', 'fee', 'priceUsd', 'feeUsd'], '请参阅官方提供方信息');
  ...
  function examFacts(track) {
    return [
      ...
      { value: passing(track), label: '通过分数' },
      { value: price(track), label: '考试费用' },
      { value: examValue(track, ['format'], '闭卷'), label: '形式' },
      { value: examValue(track, ['validityMonths', 'validForMonths'], '请参阅提供方信息'), label: '有效期' },
    ];
  }
  ...
  function renderAccessNotice(id, program, includeAllLinks) {
    ...
    mount.hidden = false;
    mount.innerHTML = '<div><strong>官方考试报名目前受限</strong><p>' + esc(notice) + '</p></div>' +
  ...
  function renderCatalog() {
    ...
    if (!certs.program || !tracks().length) {
      grid.innerHTML = '<div class="cert-empty">认证路径正在整理中。添加项目 manifest 后，请运行 <code>node site/build.js</code>。</div>';
      if (summary) summary.textContent = '本地认证目录尚未生成。';
      return;
    }

    if (title) title.textContent = 'AI 认证课程';
    if (summary) summary.textContent = '为 AI Engineering 认证提供免费、独立、开源的备考内容。先从 Claude 开始，之后还会推出更多认证系列。';
    if (meta) {
      var verified = program.verifiedAt || program.lastVerified || program.updatedAt;
      meta.innerHTML = metaChip('Claude 现已可用') +
        metaChip(tracks().length + ' 条基于角色的路径') +
        metaChip(Object.keys(certs.lessonsByPath || {}).length + ' 节认证课程') +
        metaChip(verified ? '验证于 ' + formatDate(verified) : '采用版本化来源材料');
    }
    ...
    if (notice && program.disclaimer) {
      notice.innerHTML = '<strong>独立备考课程</strong><p>' + esc(program.disclaimer) + '</p>' +
    ...
    grid.innerHTML = tracks().map(function (track, index) {
      ...
      return '<a class="cert-track-card cert-catalog-arrival" style="--cert-arrival-delay:' + delay + 'ms" href="certification.html?id=' + encodeURIComponent(track.id) + '">' +
        '<div class="cert-card-top"><span class="cert-card-code">' + esc(track.examCode || track.shortName || track.slug) + '</span><span class="cert-status">' + esc(track.level || '学习路径') + '</span></div>' +
        '<div class="cert-card-identity' + (badge ? ' has-badge' : '') + '"><h3>' + esc(track.credential || track.title || track.shortName || track.id) + '</h3>' + badge + '</div>' +
        '<p>' + esc(track.summary || track.audience || '一条覆盖此认证考试蓝图的实践路线。') + '</p>' +
        '<div class="cert-card-facts">' + renderCardFacts(track, 3) + '</div>' +
        '<div class="cert-card-footer"><span>' + lessonCount + ' 节课程 · ' + domains + ' 个领域</span><span>打开路径 →</span></div>' +
      '</a>';
    }).join('');
  }
  ...
  function renderTrackSeo(track) {
    ...
    var description = track.summary || '包含课程和原创练习的免费独立认证备考内容。';
  ...
  function renderTrackNotFound(hero) {
    document.title = '未找到认证路径 - AI Engineering from Scratch';
    var description = document.querySelector('meta[name="description"]');
    if (description) description.setAttribute('content', '请选择一条可用的认证备考路径。');
    var canonical = document.querySelector('link[rel="canonical"]');
    ...
    hero.innerHTML = '<div class="cert-track-not-found-copy"><div class="cert-eyebrow">未找到路径</div><h1>请选择一条可用的认证路径。</h1><p class="cert-track-summary">此路径 ID 与当前认证目录不匹配。</p><div class="cert-track-hero-actions"><a class="cert-action" href="certifications.html">所有认证</a></div></div>';
  }
  ...
  function renderTrack() {
    ...
    hero.innerHTML = '<div class="cert-eyebrow">' + esc(track.examCode || track.shortName || '认证路径') + '</div>' +
      ...
      '<div class="cert-track-hero-actions">' +
        ...
        '<a class="cert-action secondary" href="#trackAssessments">练习就绪度</a>' +
        '<a class="cert-action secondary" href="' + attr(TUTOR_GUIDE_URL) + '" target="_blank" rel="noopener" aria-label="在 GitHub 上与 AI 导师一起学习（在新标签页中打开）">在 GitHub 上与 AI 导师一起学习 ↗</a>' +
  ...
  function renderTrackProgress(track) {
    ...
    mount.innerHTML = '<div class="cert-progress-head"><div><div class="cert-eyebrow">本地进度</div><h2 id="progressTitle">你的进度</h2></div><div class="cert-progress-number">' + percent + '%</div></div>' +
      ...
      '<div class="cert-progress-detail"><span>已完成 ' + complete + '/' + total + ' 节课程</span><span>已尝试 ' + attempted + '/' + assessments.length + ' 项评估</span><span>' + (progressIsPersistent() ? '仅存储在此浏览器中' : '浏览器存储不可用；进度仅为临时数据') + '</span></div>';
  }
  ...
  function renderDomains(track) {
    ...
    }).join('') : '<div class="cert-empty">此路径的 manifest 中尚无领域详情。</div>';
  }
  ...
  function renderAssessments(track) {
    ...
    mount.innerHTML = assessments.length ? assessments.map(function (meta) {
      ...
      return '<article class="cert-assessment-card"><div class="cert-assessment-head"><span class="cert-assessment-kind">' + esc(assessment.kind || '练习') + '</span><span class="cert-card-code">' + count + ' 道题' + (limit ? ' · ' + limit + ' 分钟' : '') + '</span></div><h3>' + esc(assessment.title || meta.title || '练习评估') + '</h3><p>' + esc(assessment.summary || assessment.description || '提交后提供反馈的原创场景练习。') + '</p><div class="cert-assessment-best">' + (best ? '最佳练习得分：' + best.percent + '%' : '尚未在此设备上尝试') + '</div><div class="cert-track-hero-actions"><a class="cert-action" href="assessment.html?id=' + encodeURIComponent(meta.id) + '">' + (latest ? '再次尝试' : '开始练习') + '</a>' + (latest ? '<a class="cert-action secondary" href="assessment.html?id=' + encodeURIComponent(meta.id) + '&result=latest">查看最近结果</a>' : '') + '</div></article>';
    }).join('') : '<div class="cert-empty">此路径的练习评估正在准备中。</div>';
  }
  ...
  function renderStudyPlans(track) {
    ...
    mount.innerHTML = plans.length ? plans.map(function (plan) {
      ...
      var workload = plan.hoursPerWeek ? '每周 ' + plan.hoursPerWeek + ' 小时' : '';
    ...
    }).join('') : '<div class="cert-empty">请将上方的课程顺序作为默认学习计划。</div>';
  }
  ...
  function renderAssessmentError(mount) {
    mount.innerHTML = '<div class="cert-error"><h1>未找到评估</h1><p>生成的认证数据中不包含此练习集。</p><a class="cert-action" href="certifications.html">所有认证</a></div>';
  }

  function renderAssessmentLoaded(mount, assessment, track, forceForm) {
    var questions = (assessment.questions || []).map(normalizeQuestion);
    if (!questions.length) {
      mount.innerHTML = '<div class="cert-error"><h1>练习正在编写中</h1><p>此评估已有元数据，但尚未添加题目。</p><a class="cert-action" href="' + (track ? 'certification.html?id=' + encodeURIComponent(track.id) : 'certifications.html') + '">返回路径</a></div>';
      return;
    }
    ...
    mount.innerHTML = '<div class="assessment-shell"><div class="cert-breadcrumb"><a href="certifications.html">认证</a><span>/</span>' + (track ? '<a href="certification.html?id=' + encodeURIComponent(track.id) + '">' + esc(track.examCode || track.shortName || track.id) + '</a><span>/</span>' : '') + '<span>练习</span></div><section class="assessment-hero"><div class="cert-eyebrow">' + esc(assessment.kind || '练习') + '</div><h1>' + esc(title) + '</h1><p>' + esc(assessment.summary || assessment.description || '完成这些原创场景，然后提交答案以查看解释和领域反馈。') + '</p><p class="assessment-scoring-note">准备好后可随时提交。未回答的题目将计为错误。</p><div class="cert-meta-row">' + metaChip(questions.length + ' 道原创题目') + metaChip(limit ? limit + ' 分钟限制' : '不限时') + metaChip(progressIsPersistent() ? '已保存到本地' : '进度仅在此页面有效') + '</div></section>' +
      '<div class="cert-notice"><strong>仅为练习得分</strong><p>你的结果是由这套开源课程计算的百分比。它不是官方换算考试分数、认证决定，也不保证通过考试。</p></div>' +
      ...
      '<form class="assessment-form" id="assessmentForm" tabindex="-1" aria-label="评估题目">' + questions.map(function (question, index) { return renderQuestion(question, index, draft.answers[question.id] || []); }).join('') + '<div class="cert-submit-row"><p>你可以提交未完成的尝试。未回答的题目将计为错误，提交后会显示解释。</p><button class="cert-action" type="submit">提交练习</button></div></form></div>';
  ...
  function renderQuestion(question, index, selected) {
    ...
    return '<fieldset class="cert-question"><legend><div class="cert-question-head"><span class="cert-question-kicker">第 ' + (index + 1) + ' 题 · ' + esc(question.domain) + '</span><span class="cert-question-kicker">' + (question.type === 'multiple' ? '选择所有适用项' : '选择一项') + '</span></div><div class="cert-question-prompt">' + esc(question.prompt) + '</div></legend><div class="cert-option-list">' + question.options.map(function (option, optionIndex) {
  ...
  function renderRemediation(track, questions, attempt) {
    ...
    var cards = missed.map(function (item) {
      ...
      if (!links) {
        links = '<li><a href="#' + attr(item.reviewIds[0]) + '">查看答错题目的解释 →</a></li>';
      }
    ...
    return '<section class="cert-remediation" aria-labelledby="remediationTitle"><div class="cert-eyebrow">针对性补强</div><h2 id="remediationTitle">学习你遗漏的决策要点</h2><p>下方每节课程都来自你答错题目的内部引用。重新学习相关决策，然后开始一次新的尝试。</p><div class="cert-remediation-grid">' + cards + '</div></section>';
  }
  ...
  function renderAssessmentResults(mount, assessment, track, questions, attempt) {
    ...
    mount.innerHTML = '<div class="assessment-shell cert-results"><div class="cert-breadcrumb"><a href="certifications.html">认证</a><span>/</span>' + (track ? '<a href="certification.html?id=' + encodeURIComponent(track.id) + '">' + esc(track.examCode || track.shortName || track.id) + '</a><span>/</span>' : '') + '<span>结果</span></div><section class="cert-results-summary" id="assessmentResultsSummary" tabindex="-1" aria-label="评估结果"><div class="cert-results-head"><div><div class="cert-eyebrow">练习结果</div><div class="cert-results-score">' + scoreSummary.percent + '%</div></div><div><strong>' + scoreSummary.correct + '/' + scoreSummary.total + ' 道题完全正确</strong><p>' + (attempt.timedOut ? '计时结束时已自动提交。' : '用时 ' + formatDuration(attempt.durationMs) + ' 提交') + '</p></div></div><div class="cert-domain-score-grid">' + domainHtml + '</div><div class="cert-notice"><strong>并非官方换算分数</strong><p>此百分比仅衡量这套原创练习。它无法预测或保证提供方的换算认证结果。</p></div><div class="cert-track-hero-actions"><button class="cert-action" type="button" id="assessmentRetake">开始新的尝试</button><a class="cert-action secondary" href="' + attr(latestResultHref) + '">再次查看最近结果</a>' + (track ? '<a class="cert-action secondary" href="certification.html?id=' + encodeURIComponent(track.id) + '">返回路径</a>' : '') + '</div></section>' + remediationHtml + '<section aria-labelledby="reviewTitle"><div class="cert-section-heading"><div><div class="cert-eyebrow">复盘</div><h2 id="reviewTitle">决策与解释</h2></div><p>利用每一道错题，找出需要重新学习的领域和决策规则。</p></div>' + questions.map(function (question, index) { return renderReview(question, index, attemptAnswers[question.id] || [], track); }).join('') + '</section></div>';
