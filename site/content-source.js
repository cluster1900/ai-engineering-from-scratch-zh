/**
 * 解析静态页面所需的 repository 内容。
 *
 * 本地预览从 repository 根目录提供，因此 ../ 可直接访问课程
 * 和评估源文件。部署环境通过 build-meta.js 保留现有的分支感知
 * raw GitHub 行为。
 */
(function () {
  'use strict';
  ...
  function localDirectoryFiles(path) {
    if (!isLocal()) return Promise.reject(new Error('本地目录列表不可用'));

    var safe = clean(path).replace(/\/+$/, '');
    if (!safe || /\\/.test(safe)) return Promise.reject(new Error('本地目录路径无效'));
    ...
    return fetch(expectedUrl.href, { headers: { 'Accept': 'text/html' } }).then(function (res) {
      ...
      return res.text().then(function (html) {
        ...
        if (!/(?:directory listing for|index of)/i.test(indexLabel)) {
          throw new Error('本地 server 未开放目录列表');
        }
