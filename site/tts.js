...
(function () {
  ...
  function render() {
    ...
    if (isWaiting()) {
      els.status.textContent = '正在加载下一节课…';
  ...
  /**
   * A "从此处开始朗读" chip that follows a text selection inside the article.
   */
  function buildSelectionButton() {
    ...
    btn.id = 'ttsFromHere';
    btn.hidden = true;
    btn.innerHTML = '<span aria-hidden="true">▶</span> 从此处开始朗读';
    btn.title = '从此处开始朗读 (Alt+R)';
