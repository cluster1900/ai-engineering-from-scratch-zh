...
function parseRoadmap(content) {
  const statuses = {}; // { "Phase 0": { phaseStatus, lessons: { "开发环境": "complete" } } }
...
// The public Mermaid diagram under "课程体系的结构" owns the
...
function parseCurriculumPrereqs(content, phases) {
  const section = content.match(/## (?:The shape of the curriculum|课程体系的结构)[\s\S]*?```mermaid\s*\r?\n([\s\S]*?)```/);
  if (!section) throw new Error('README.md 缺少规范的课程体系 Mermaid 图');
  ...
  if (!seenEdges.size) throw new Error('规范的课程体系 Mermaid 图不包含任何边');
...
function assertCertificationTrackOrder(program, tracks) {
  ...
  if (!matches) {
    throw new Error(
      ...
      JSON.stringify(declaredTrackIds) + '，但 Track manifest 输出 ' +
...
function parseCertifications() {
  ...
  const program = readJson(path.join(programDir, 'program.json'), '认证项目');
  ...
  for (const track of tracks) {
    track.assessments = track.assessments.map((meta, index) => {
      ...
      const merged = {
        ...
        title: normalized.title || data.title || '练习评估',
...
const GLOSSARY_CATEGORY_ALIASES = new Map([
  ['Model 与 Inference', 'Models 与 Inference'],
  ['数据与表示', '数据与表征'],
  ['Prompt 与 Context', 'Prompting 与 Context'],
  ['Agent 与 Tool', 'Agents 与 Tools'],
  ['基础设施与 Serving', '基础设施与服务'],
]);
...
function parseGlossary(content) {
  ...
  function finishEntry() {
    ...
    if (!currentTerm.category) {
      glossaryError(currentTerm.headerLine, currentTerm.term, '缺少必填字段 "Category"');
    }
    if (!currentTerm.means) {
      glossaryError(currentTerm.headerLine, currentTerm.term, '缺少必填字段 "What it actually means"');
    }
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (/^###\s*$/.test(line)) glossaryError(lineNumber, '', '术语标题不能为空');
    ...
    if (termMatch) {
      ...
      if (!slug) glossaryError(lineNumber, term, '术语必须至少包含一个字母或数字');
    ...
    if (fieldMatch) {
      ...
      if (currentTerm.fields.has(key)) glossaryError(lineNumber, currentTerm.term, `duplicate field "${label}"`);
    ...
    if (trimmed && !trimmed.startsWith('<!--')) {
      glossaryError(lineNumber, currentTerm.term, '应为规范的项目符号字段或下一个术语标题');
    }
  }
...
function build() {
  ...
  console.log('📚 正在从 docs/en.md 提取课程摘要和关键词...');
