(function () {
  ...
  function bpeMerge(host) {
    ...
    card(host, 'BPE 合并循环', '扫描 · 合并 · 重复',
      svg,
      'Byte-Pair Encoding 从原始字节开始，反复寻找最高频的相邻字节对，将其合并为一个新符号，并为词表增加一个 id。扫描框会遍历每一对相邻字节；胜出的字节对会折叠为单个 Token。');
  }

  // ── 滑动窗口：固定窗口在 id 流上滑动，每步包含 T+1 个 id ───────────────
  function slidingWindow(host) {
    ...
    var i;
    svg.appendChild(txt(20, 50, 'Token id 流', MUTE, 10, 'start'));
    for (i = 0; i < n; i++) {
      var x = x0 + i * (bw + 4);
    ...
    wg.appendChild(txt(x0 - 3 + step + (inW + 6) / 2, y + 80, '目标 = 输入偏移一个位置', WARN, 10, 'middle'));
    wg.appendChild(animT('translate', '0 0;' + maxShift + ' 0', '6s', { calcMode: 'discrete' }));
    svg.appendChild(wg);
    card(host, '滑动窗口', '窗口滑动 · 步幅',
      svg,
      'Tokenizer 将语料展平为一条长 id 流。包含 T 个 id 的定长窗口按指定步幅沿流滑动，目标则是同一窗口向左偏移一个位置，用于预测下一个 Token。步幅越小，窗口重叠越多，样本数量也会被放大。');
  }

  // ── Multi-Head Attention：一次投影分流为 H 个并行 head ─────────────────
  function multiHead(host) {
    ...
    card(host, 'MULTI-HEAD ATTENTION', '一次投影 · H 个 head',
      svg,
      '一个 Linear layer 将输入投影为 Q、K 和 V，再 reshape 为 H 个并行 head，每个 head 的大小为 D/H。每个 head 在 causal mask 下独立执行 scaled dot-product Attention，输出再拼接回 D。各个 head 会在 Training 期间形成专门分工。');
  }

  // ── Training 循环：forward、backward、step，然后 Loss 下降 ──────────────
  function trainingLoop(host) {
    ...
    card(host, 'TRAINING 循环', 'forward · backward · step',
      svg,
      '每个 step 获取一个 Batch，执行 forward pass 得到 logits，计算 cross-entropy Loss，运行 Backpropagation，并按照 LR schedule 执行一次 AdamW step。如此重复数千次后，Loss 曲线会下降并逐渐趋平。定期 Evaluation 和样本探测可以发现标量 Loss 所掩盖的发散问题。');
  }

  // ── classifier head 替换：冻结主体、移除旧 head、装上新的二分类 head ───
  function headSwap(host) {
    ...
    svg.appendChild(txt(bx + bw / 2, by + 22, 'Transformer 主体', SOFT, 11));
    svg.appendChild(txt(bx + bw / 2, by + 38, '（已冻结）', MUTE, 10));
    var ly;
    for (ly = 0; ly < 4; ly++) {
      var yy = by + 58 + ly * 24;
    ...
    oldH.appendChild(txt(355, 60, '旧 LM head', MUTE, 11));
    oldH.appendChild(txt(355, 76, '词表投影', MUTE, 9));
    ...
    newH.appendChild(txt(355, 152, '新 classifier head', BP, 10));
    newH.appendChild(txt(355, 168, 'Linear → 2 个 logits', SOFT, 9));
    ...
    card(host, 'CLASSIFIER HEAD 替换', '冻结主体 · 替换 head',
      svg,
      '预训练 Model 由冻结的主体和一个 head 组成。执行 Classification 时，保留成本高昂的主体，移除词表 head，再接入一个输出两个 logits 的小型 Linear layer。若追求速度，可仅 Training head；当领域发生漂移且需要提高准确率时，则可解除主体的冻结状态。');
  }

  // ── DPO：围绕冻结的 reference，提高 chosen log-prob，降低 rejected log-prob ──
  function dpoPreference(host) {
    ...
    svg.appendChild(gap);
    card(host, 'DPO 偏好', 'chosen ↑ · rejected ↓',
      svg,
      'DPO 直接使用 chosen-over-rejected 样本对 Training policy，无需单独的 reward model。Loss 是相对于冻结 reference 的 log-probability 差值经过 sigmoid 后的结果，因此 chosen completion 的概率会被推高，rejected completion 的概率会被拉低，而 KL anchor 则约束 policy 不要偏离过远。');
  }

  // ── 语料下载器：流式接收 shard、断线续传、去重、生成 manifest ────────
  function corpusStream(host) {
    ...
    svg.appendChild(txt(59, 124, 'shard', SOFT, 10));
    // 字节流动的管道
    var pipeY = 112;
    ...
    svg.appendChild(txt(280, 124, '流', SOFT, 8));
    // 去重 bucket：文档落入 LSH bucket，近重复项被移除
    var bx = 340, j;
    for (j = 0; j < 3; j++) {
      var bxj = bx + j * 56;
    ...
    svg.appendChild(txt(bx + 56, 36, '重复项已丢弃', WARN, 9, 'middle'));
    // manifest
    svg.appendChild(svgEl('line', { x1: 310, y1: 112, x2: 340, y2: 112, stroke: BP, 'stroke-width': '2' }));
    card(host, '语料下载器', '流式传输 · 续传 · 去重',
      svg,
      'shard 从远程来源以流式方式传输，并通过 Zstandard 解压，无需缓冲整个文件。经过验证的字节偏移量可以让中断的连接通过 HTTP Range 请求继续传输；每份文档都会获得一个 MinHash signature，并由 LSH 分配到 bucket，使近重复文档发生碰撞，在进入 manifest 前被丢弃。');
  }

  // ── cosine LR warmup：标记沿 warmup 斜坡进入 cosine decay ─────────────
  function cosineWarmup(host) {
    ...
    card(host, 'COSINE LR WARMUP', '线性升高 · 逐步衰减',
      svg,
      '该 schedule 在 warmup steps 内将 learning rate 从零线性提升至 lr_max，以保护脆弱的初始更新，随后沿 cosine 曲线的上半段逐渐回落至 lr_min。标记点表示 Optimizer 在每个 step 实际使用的精确值。');
  }
