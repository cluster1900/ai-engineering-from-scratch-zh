...
该 Capstone 构建一个分层检测器：确定性子字符串规则、token 级正则表达式，以及在规则运行前解码简单编码（base64、rot13、leet、零宽度字符）的规范化通道。每一层都可独立审计。每条规则都有针对每个类别的 coverage 声明。runner 会生成每个类别的混淆矩阵，以及下游课程可以绘制的 CSV。
