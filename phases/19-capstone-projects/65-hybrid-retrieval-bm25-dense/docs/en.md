...
- 在确定性模拟嵌入之上构建密集检索器，以便循环离线运行。
- 完全按照 Cormack、Clarke 和 Buettcher 在 2009 年发表的那样实现倒数等级融合，并解释为什么它在分数加权插值中占主导地位。
- 调整 RRF k 常数和每种模态权重，并在小型 fixture 语料库上读取权衡。

## 问题

当查询携带语料库逐字包含的文字标识符时，词法搜索获胜。对 `AbortMultipartOnFail` 的查询会通过 BM25 以微秒为单位返回正确的 Go 函数。嵌入的相同查询位于三个相似性簇的边界，并且密集检索器将错误的文件排在第一位。
...
使用嵌入模型将每个块嵌入到固定维度向量中。在查询时，嵌入查询，根据相似度对每个块进行余弦排名，并返回前 k 个。模型是决定质量的变量。检索算法本身就是两行：点积和排序。

本课程使用确定性的基于哈希的嵌入，因此您无需网络调用即可阅读融合数学。该哈希将token键控偏移量求和为 96 维向量并进行归一化。余弦等级在运行中具有确定性，这是测试套件所需要的。
...
- `mock_embed`、`DenseIndex` - 与第 64 课相同的确定性嵌入，因此块具有可比性。
- `rrf(rankings, k, weights)` - 已发布的多模态权重融合。
...
**词汇外token。** BM25 的 IDF 是根据语料库计算的，因此仅查询中的术语贡献为零。密集嵌入会产生同一项向量的幻觉。对于语料库外标识符，密集模态返回看似合理但错误的邻居。融合吸收了这一点，因为 BM25 没有返回任何内容，并且排名贡献会下降，但前提是您按文档而不是按块去重复。
...
- 索引 BM25 正在处理中；瓶颈是词频字典，而不是向量。
- 在单独的存储中索引密集向量（在本课中我们使用平面列表；在生产中您将使用 HNSW）。
