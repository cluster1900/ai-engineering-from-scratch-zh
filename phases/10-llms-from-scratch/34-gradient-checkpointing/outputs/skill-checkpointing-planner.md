...
示例输入: "L=64, d=8192, S=8192, B=1, bf16, FlashAttention-3, TP=8, PP=4，weights 后每个 rank 的 HBM budget 为 32 GB，MoE 有 8 个 experts 且 EP=8。"

示例输出:
- Per-layer policy: attention selective，FFN none，MoE expert full，embedding none，output head offload。
- Segment size: full 只应用于 MoE，k=8；expert path 上 FLOP overhead 为 12 percent，其他地方为 0。
...
- Budget math: 无 policy 时 activations 为 38 GB，有 policy 时为 11 GB。Total FLOP overhead 为 fwd+bwd 的 7.5 percent。
