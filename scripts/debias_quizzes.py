...
def debias_question(path, q):
    ...
    if not (0 <= correct < len(options)) or len(options) < 2:
        return False
    if CJK.search(str(q.get("question", ""))) or any(CJK.search(str(option)) for option in options):
        # Localized text changes the content seed. Preserve the already-debiased
        # upstream order and answer index instead of reshuffling by translation.
        return False
