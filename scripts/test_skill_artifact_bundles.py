...
class TutorSkillCompatibilityTest(unittest.TestCase):
    def test_mcp_tutor_migrates_legacy_state_and_skill_mirrors_match(self) -> None:
        ...
        self.assertTrue(
            "rename the legacy file to `MCP-LEARNING.md`" in focused
            or "将 legacy 文件重命名为同一目录中的 `MCP-LEARNING.md`" in focused
        )
        self.assertTrue(
            "Preserve every learner note and evidence row byte for byte" in focused
            or "逐字节保留每条学习者笔记和证据行" in focused
        )
        self.assertTrue(
            "`MCP-ENGINEERING-LEARNING.md` exists" in generic
            or "`MCP-ENGINEERING-LEARNING.md` 是同一条 `learn-mcp` 路线的旧版文件名" in generic
        )
        self.assertTrue(
            "without discarding learner evidence" in generic
            or "同时保留学习者证据" in generic
        )
