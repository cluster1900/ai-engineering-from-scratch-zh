...
LEARNING_OBJECTIVES_RE = re.compile(r"^##\s+(?:Learning Objectives|学习目标)\s*$", re.MULTILINE)
...
PARITY_HEADING_ALIASES = {
    "Interactive Lab": ("Interactive Lab", "交互实验", "交互式实验", "交互式 Lab"),
    "Practice Lab": ("Practice Lab", "实践实验", "实践 Lab"),
    "Shipped Artifact": ("Shipped Artifact", "交付产物", "交付成果"),
    "Verify It": ("Verify It", "验证", "验证它"),
    "Capstone Connection": (
        "Capstone Connection", "Capstone 关联", "Capstone 衔接", "Capstone 联系",
        "综合项目关联", "综合项目衔接", "与综合项目的联系",
    ),
}
EXPECTED_FIGURES = {
...
def prose_units(text: str) -> int:
    """Approximate semantic length across English and Simplified Chinese."""
    latin_words = len(re.findall(r"[A-Za-z0-9]+", text))
    cjk_chars = len(re.findall(r"[\u3400-\u9fff]", text))
    return latin_words + (2 * cjk_chars + 2) // 3


def option_word_units(text: str) -> int:
    latin_words = len(re.findall(r"[A-Za-z0-9]+", text))
    cjk_runs = len(re.findall(r"[\u3400-\u9fff]+", text))
    return latin_words + cjk_runs


def states_non_affiliation(text: str) -> bool:
    lowered = text.lower()
    return "not affiliated" in lowered or "不存在从属关系" in text or "没有隶属关系" in text
...
def check_answer_quality(audit: Audit, path: Path, questions: list[Any]) -> None:
    ...
    for question in questions:
        ...
        if not has_cjk:
            metrics.append(("words", [option_word_units(str(option)) for option in options]))
        ...
        if len(correct) == 1:
            single_count += 1
            lengths = [len(str(option)) for option in options] if has_cjk else [option_word_units(str(option)) for option in options]
...
def check_lesson(audit: Audit, lesson_dir: Path) -> None:
    ...
    if units < 800:
        audit.add("C020", doc_path, f"lesson is too thin for certification preparation: {units} prose units, minimum 800")
    ...
    for heading in PARITY_HEADINGS:
        aliases = PARITY_HEADING_ALIASES[heading]
        if not any(re.search(rf"^##\s+{re.escape(alias)}\s*$", text, re.MULTILINE) for alias in aliases):
...
def check_assessment_question(
    ...
    if not isinstance(question.get("explanation"), str) or prose_units(question["explanation"]) < 20:
...
def check_track(audit: Audit, path: Path, track: Any, global_ids: set[str]) -> tuple[dict[str, Any] | None, set[str]]:
    ...
    if not isinstance(exam, dict):
        ...
        for field, expected in expected_exam.items():
            actual = exam.get(field)
            accepted = {expected}
            if field == "effective" and expected == "July 2026":
                accepted.add("2026 年 7 月")
...
def check_ai_native_learning_surface(audit: Audit, actual_lessons: set[str]) -> None:
    ...
    guide_tokens = (
        ...
        ("not affiliated", "不存在从属关系", "没有隶属关系"),
        ("not included in the repository's EPUB/PDF book workflow", "没有纳入代码仓库的 EPUB/PDF 图书工作流"),
    )
    for token in guide_tokens:
        alternatives = token if isinstance(token, tuple) else (token,)
        if not any(candidate in guide for candidate in alternatives):
            audit.add("C080", GETTING_STARTED_PATH, f"AI-native learner guide is missing one of {alternatives!r}")
    ...
    skill_tokens = (
        ...
        ("Never invent fake API code", "虚构 API 代码"),
        ("book-generation pipeline", "图书生成 Pipeline", "书籍生成流水线"),
    )
    for token in skill_tokens:
        alternatives = token if isinstance(token, tuple) else (token,)
        if not any(candidate in skill for candidate in alternatives):
            audit.add("C080", CERT_SKILL_PATH, f"certification tutor skill is missing one of {alternatives!r}")
    ...
    readme_tokens = (
        ...
        "skills/claude-certification/SKILL.md",
        "CLAUDE-CERTIFICATION.md",
        ("intentionally not converted into the books", "有意不纳入 EPUB/PDF 图书工作流"),
    )
    for token in readme_tokens:
        alternatives = token if isinstance(token, tuple) else (token,)
        if not any(candidate in root_readme for candidate in alternatives):
            audit.add("C080", ROOT_README_PATH, f"root README is missing certification onboarding text {alternatives!r}")

    for token in (
        "GETTING_STARTED.md",
        "../../skills/claude-certification/SKILL.md",
        ("outside the EPUB/PDF book workflow", "独立于 EPUB/PDF 图书工作流"),
    ):
        alternatives = token if isinstance(token, tuple) else (token,)
        if not any(candidate in cert_readme for candidate in alternatives):
            audit.add("C080", CERT_README_PATH, f"certification README is missing one of {alternatives!r}")
...
def run_audit() -> Audit:
    ...
    if not states_non_affiliation(disclaimer) or "anthropic" not in disclaimer:
    ...
    for page in PUBLIC_CERT_PAGES:
        if not page.is_file() or not states_non_affiliation(page.read_text(encoding="utf-8")):
    ...
    readme_counts = {
        code: int(count)
        for code, count in re.findall(r"^\|\s*(CC[A-Z-]+)\s*\|.*\|\s*(\d+)\s+(?:lessons|节课)\s*\|$", readme_text, re.MULTILINE)
    }
