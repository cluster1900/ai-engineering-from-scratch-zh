...
def rewrite_question(
    ...
    if re.search(r"[\u3400-\u9fff]", question_text(question)) or any(
        re.search(r"[\u3400-\u9fff]", option) for option in options
    ):
        # The canonical English bank was already position-balanced. Translation
        # changes content hashes, so keep its option order and correct indices.
        return False, None
