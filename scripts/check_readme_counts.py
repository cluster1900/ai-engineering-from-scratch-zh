...
PATTERNS: tuple[CountPattern, ...] = (
    ...
    CountPattern(
        regex=re.compile(r'alt="(\d+)(?: lessons| 节课)"'),
        field="lessons",
        description="lesson-count badge alt text",
    ),
    CountPattern(
        regex=re.compile(r"^> (\d+)(?: lessons\. \d+ phases\.| 节课。\d+ 个阶段。)", re.MULTILINE),
        field="lessons",
        description="hero blockquote lesson count",
    ),
    CountPattern(
        regex=re.compile(r"^> (?:\d+ lessons\. |\d+ 节课。)(\d+)(?: phases\.| 个阶段。)", re.MULTILINE),
        field="phases",
        description="hero blockquote phase count",
    ),
    CountPattern(
        regex=re.compile(r"(?:This curriculum is the spine\. (\d+) phases,|这套课程体系就是主干。(\d+) 个阶段，)"),
        field="phases",
        description="'spine' prose phase count",
    ),
    CountPattern(
        regex=re.compile(r"(?:This curriculum is the spine\. \d+ phases, (\d+) lessons,|这套课程体系就是主干。\d+ 个阶段，(\d+) 节课，)"),
        field="lessons",
    ...
    CountPattern(
        regex=re.compile(r"(?:portfolio of (\d+) artifacts|包含 (\d+) 个产物的作品集)"),
        field="lessons",
        description="'portfolio of N artifacts' (one artifact per lesson)",
    ),
    CountPattern(
        regex=re.compile(r"(?:The repo ships (\d+) skills|此 repo 在以下位置提供 (\d+) 个 Skills)"),
        field="skills",
        description="toolkit section skill count",
    ),
    CountPattern(
        regex=re.compile(r"(?:The repo ships \d+ skills and (\d+) prompts|此 repo 在以下位置提供 \d+ 个 Skills 和 (\d+) 个 Prompts)"),
        field="prompts",
        description="toolkit section prompt count",
    ),
    CountPattern(
        regex=re.compile(r"(?:MIT-licensed, (\d+) lessons\.|采用 MIT 许可、包含 (\d+) 节课。)"),
        field="lessons",
...
def numeric_capture(match: re.Match[str]) -> tuple[int, str]:
    for index, value in enumerate(match.groups(), start=1):
        if value is not None:
            return index, value
    raise ValueError(f"count pattern has no captured value: {match.re.pattern!r}")
...
def find_mismatches(readme_text: str, totals: dict[str, int]) -> list[Mismatch]:
    ...
    for pattern in PATTERNS:
        ...
        for match in pattern.regex.finditer(readme_text):
            matched_any = True
            _, captured = numeric_capture(match)
            found = int(captured)
            if found != expected:
                mismatches.append(
                    Mismatch(
...
def apply_fixes(readme_text: str, totals: dict[str, int]) -> str:
    for pattern in PATTERNS:
        ...
        def replace(match: re.Match[str], expected: int = expected) -> str:
            whole = match.group(0)
            group_index, old = numeric_capture(match)
...
def is_localized_display(*values: str) -> bool:
    """Return true when presentation text is intentionally localized to Chinese."""
    localized_terms = {"LLM"}
    return any(
        re.search(r"[\u4e00-\u9fff]", value) or value in localized_terms
        for value in values
    )
...
def check_book_volumes() -> list[str]:
    ...
    for vol in volumes:
        ...
        if not m or "aiefs-vol" not in m.group(3):
            ...
            if (
                m.group(1) != f"{title} · {subtitle}"
                and not is_localized_display(m.group(1))
            ):
                errors.append(
        ...
        if not m:
            ...
            if m.group(1) != title and not is_localized_display(m.group(1)):
                errors.append(
        ...
        if not m:
            ...
            if (
                (m.group(2) != title or m.group(3) != subtitle)
                and not is_localized_display(m.group(2), m.group(3))
            ):
                errors.append(
